...
-(void)testInvalidSizeShouldThrow {
	ArgsParser parser{config};
	XCTAssertThrows(parser.parse({"program", "1", "invalid", "1,2,3,_", "1,2,3,_"}));
	XCTAssertThrows(parser.parse({"program", "1", "3xa", "1,2,3,_", "1,2,3,_"}));
	XCTAssertThrows(parser.parse({"program", "1", "3", "1,2,3,_", "1,2,3,_"}));
	XCTAssertThrows(parser.parse({"program", "1", "axa", "1,2,3,_", "1,2,3,_"}));
	XCTAssertThrows(parser.parse({"program", "1", "0x0", "1,2,3,_", "1,2,3,_"}));
}

-(void)testInvalidStateShouldThrow {
	ArgsParser parser{config};
	XCTAssertThrows(parser.parse({"program", "1", "2x2", "_,_", "1,2,3,_"}));
	XCTAssertThrows(parser.parse({"program", "1", "2x2", "1,2", "1,2,3,_"}));
	XCTAssertThrows(parser.parse({"program", "1", "2x2", "a,b", "1,2,3,_"}));
	XCTAssertThrows(parser.parse({"program", "1", "2x2", "1,_", "1,2,3,_"}));
}

-(void)testInvalidHeuristicsShouldThrow {
	ArgsParser parser{config};
	XCTAssertThrows(parser.parse({"program", "a", "2x2", "1,2,3,_", "1,2,3,_"}));
	XCTAssertThrows(parser.parse({"program", "4", "2x2", "1,2,3_", "1,2,3,_"}));
}

-(void)testShouldParseValidArguments {
	ArgsParser parser{config};
	ArgsParser::Args args = parser.parse({"program", "1", "2x2", "1,2,3,_", "1,_,3,2"});
	...
	XCTAssertTrue(args.heuristics != NULL);
}

-(void)testShouldParseValidArgumentsWithConfig {
	const ArgsParser::Config c = {"|", " ", -5};
	ArgsParser parser{c};
	ArgsParser::Args args = parser.parse({"program", "1", "2x2", "1|2|3| ", "1| |3|2"});
	XCTAssertEqual(2, args.start.size.width);
	XCTAssertEqual(2, args.start.size.height);
	XCTAssertEqual(4, args.start.tiles.size());
